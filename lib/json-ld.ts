...
export function generatePersonSchema(siteUrl: string = SITE_URL): PersonSchema {
  return {
    ...
    name: 'IFRAP Programme (FPMU / PIU)',
    jobTitle: 'Implementation, Safeguards & M&E Unit',
    description:
      'Provincial implementation unit operating the MIRAB platform for the World Bank IFRAP programme — Balochistan water governance, ESF safeguards, and monitoring & evaluation.',
    url: siteUrl,
...
export function generateProfessionalServiceSchema(siteUrl: string = SITE_URL): ProfessionalServiceSchema {
  return {
    ...
    description:
      'Cross-cutting monitoring, results and safeguards platform for the World Bank IFRAP programme (Balochistan): field ethnography, Grievance Redress, customary-tenure certificates, participatory WebGIS, ESF safeguard telemetry, and Senian MPI M&E.',
    url: siteUrl,
    areaServed: 'Balochistan, Pakistan',
    knowAbout: [
      'Water Governance',
    ...
    offerCatalog: {
      '@type': 'OfferCatalog',
      name: 'IFRAP Operations & Results Backbone Modules',
      itemListElement: [
        {
          '@type': 'OfferCatalogItem',
          item: {
            '@type': 'Service',
            name: 'Participatory WebGIS & Spatial Analysis',
            description:
              'Interactive cartographic mapping of Karez water systems and customary-tenure spatial layers.',
          },
        },
        {
          '@type': 'OfferCatalogItem',
...
export function generateDatasetSchema(siteUrl: string = SITE_URL): DatasetSchema {
  return {
    ...
    '@id': `${siteUrl}/#dataset`,
    name: 'MIRAB — Balochistan Karez WebGIS & M&E Telemetry (IFRAP Programme)',
    description:
      'Integrated spatial dataset, Senian MPI capability metrics, and customary-tenure rights telemetry for water resource management in Balochistan.',
    ...
    variableMeasured: [
      ...
      'Traditional & Customary Knowledge Layers',
    ],
    provider: {
      '@type': 'Organization',
      name: 'IFRAP Programme (FPMU / PIU)',
...
export function generateWebSiteSchema(siteUrl: string = SITE_URL): WebSiteSchema {
  return {
    ...
    url: siteUrl,
    description:
      'Cross-cutting monitoring, results and safeguards platform for the World Bank IFRAP programme (Balochistan): participatory WebGIS, Senian MPI M&E telemetry, Grievance Redress, and a customary-tenure fiduciary ledger.',
    publisher: {
      '@type': 'Organization',
      name: 'IFRAP Programme (FPMU / PIU)',
    },
