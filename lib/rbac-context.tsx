...
export const RoleGate: React.FC<RoleGateProps> = ({
  ...
  if (!hasPermission(allowedRoles)) {
    ...
    return (
      <div className="p-4 rounded-md bg-red-50 border border-red-200 text-sm text-red-900 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div>
          <strong className="font-bold uppercase tracking-wider text-red-700">Access Restricted:</strong>{' '}
          This section contains sensitive fiduciary data accessible only to authorized roles ({allowedRoles.join(', ')}).
        </div>
      </div>
    );
  }
